#include "PPOCRv4Model.hpp"
#include "ui/FormPPOCRv4.h"
#include "ui/popup/DialogPPOCRv4.h"
...
QWidget* PPOCRv4Model::embeddedWidget()
{
  return form;
}

void PPOCRv4Model::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void PPOCRv4Model::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void PPOCRv4Model::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &PPOCRv4Model::runProcess, _flowDataIn);
